...
##########################################################
    ...
    def __add__(self, lesson):
        lesson_day = lesson.term.day
        lesson_time = lesson.term.count_time
        #Nadpisujemy lekcje
        self.table[lesson_day.value][1][lesson_time] = deepcopy(lesson)

    # Przeladowanie -
    def __sub__(self, lesson):
        lesson_term = lesson.term
        lesson_day = lesson_term.day
        lesson_time = lesson.term.count_time
        table_slot = self.table[lesson_day.value][1][lesson_time]
        # Jesli nie ma w tym terminie lekcji, komunikat bledu
        if type(table_slot) == type(""):
            print("\t=== W tym terminie nie ma zadnych zajec! ===")
            return False
        else:
            self.table[lesson_day.value][1][lesson_time] = ""
